...
export function steerGoal(a:Point,b:Point,r:number,obstacles:Obstacle[]=OBSTACLES,terrain?:CharTerrain):Point {
 ...
 for(const o of obstacles)for(const sx of [-1,1])for(const sz of [-1,1]){
  ...
  if(clearLine(a,c,r,obstacles,terrain)) {const s=distance(a,c)+distance(c,b)+(clearLine(c,b,r,obstacles,terrain)?0:8);if(s<score){score=s;best=c;}}
 }
 return best??b;
}
