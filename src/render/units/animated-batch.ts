...
export class AnimatedBatch {
   ...
   const materials=(Array.isArray(n.material)?n.material:[n.material]).map(m=>{const mat=m.clone() as THREE.MeshStandardMaterial;
    mat.onBeforeCompile=shader=>{
     ...
     shader.vertexShader=shader.vertexShader.replace('#include <common>',`#include <common>
      attribute vec4 unitPose; attribute vec4 skinIndex; attribute vec4 skinWeight;
    ...
    };mat.customProgramCacheKey=()=> 'sc2-original-gpu-bones-v2';return mat;});
   const mesh=new THREE.InstancedMesh(geometry,materials.length===1?materials[0]:materials,CAPACITY);mesh.frustumCulled=false;mesh.count=0;mesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);scene.add(mesh);this.meshes.push(mesh);this.attributes.push(pose);
  }
 ...
 add(x:number,y:number,z:number,facing:number,action:keyof ReturnType<typeof mapAnimations>,seconds:number,once=false,hit=0,shrink=1,interpolate=true){
  ...
  this.meshes.forEach((m,i)=>{m.setMatrixAt(this.count,object.matrix);this.attributes[i].setXYZW(this.count,p.offset+a,p.offset+b,interpolate?frame-a:0,hit);});this.count++;
 }
 end(){this.meshes.forEach((m,i)=>{m.count=this.count;m.visible=this.count>0;m.instanceMatrix.needsUpdate=true;this.attributes[i].needsUpdate=true;});}
}
