...
import {healthReadout,rankLabel,specialUnitName} from '../../ui/unit-identity';
...
export class FriendlyLabels {
 ...
 update(world:World,project:(unit:Entity)=>{x:number;y:number;visible:boolean},width:number){
  ...
  for(const u of units){let label=this.labels.get(u.id);const hero=!!u.heroId;
   ...
   const name=specialUnitName(u),title=`${hero?'★':'◆'} ${name} ${rankLabel(u.rank)}`;if(label.title.textContent!==title)label.title.textContent=title;
   ...
   const w=hero?116:110,h=hero?37:19,x=Math.max(w/2+2,Math.min(width-w/2-2,p.x));let y=p.y;
