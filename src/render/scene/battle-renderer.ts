...
export class BattleRenderer {
 ...
 async load(progress:(label:string)=>void){await MeshoptSimplifier.ready;const loader=new GLTFLoader();
  ...
  if(this.world.terrain?.definition){this.mapView=await createOriginalMap(this.scene,this.world,progress);this.terrainUpdate=this.mapView.update;}else this.terrainUpdate=await createTerrain(this.scene,this.world);
