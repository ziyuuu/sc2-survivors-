...
export interface MapView {update:()=>void;report:()=>{name:string;models:number;placements:number;visibleInstances:number;area:number;scale:number;sourceSha256:string}}
export async function createOriginalMap(scene:THREE.Scene,w:World,progress:(s:string)=>void):Promise<MapView>{
 ...
 for(const [key,placements] of grouped){const [id,pose]=key.split('|');progress(`载入战场 ${++models} / ${grouped.size}`);const g=await restoreSc2Materials(await loader.loadAsync(url(id))),mixer=new THREE.AnimationMixer(g.scene),clip=g.animations.find(c=>pose==='dead'?/death|dead/i.test(c.name):/^stand|idle/i.test(c.name));if(clip){mixer.clipAction(clip).play();mixer.setTime(pose==='dead'?Math.max(0,clip.duration-.001):0);}g.scene.updateMatrixWorld(true);
  ...
  g.scene.traverse(n=>{if(!(n instanceof THREE.Mesh)||!n.visible)return;const geo=n.geometry.clone(),pos=geo.getAttribute('position');for(let i=0;i<pos.count;i++){n.getVertexPosition(i,v).applyMatrix4(n.matrixWorld);pos.setXYZ(i,v.x,v.y,v.z);}geo.deleteAttribute('skinIndex');geo.deleteAttribute('skinWeight');geo.computeVertexNormals();geo.computeBoundingSphere();
   ...
   const mesh=new THREE.InstancedMesh(geo,n.material,placements.length);mesh.count=0;mesh.frustumCulled=false;mesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);const cliff=id.includes('model.map.cliff');mesh.name=cliff?'char-solid-cliff-faces':id;if(cliff)cliffGroup.add(mesh);else scene.add(mesh);batches.push({mesh,placements,matrices,points});
  });mixer.stopAllAction();mixer.uncacheRoot(g.scene);
 }
 let last='',visibleInstances=0;const update=()=>{stage.value=w.stage;const cell=`${w.stage}:${Math.floor(w.anchor.x)}:${Math.floor(w.anchor.z)}`;if(cell===last)return;last=cell;visibleInstances=0;for(const b of batches){let n=0;for(let i=0;i<b.points.length;i++){const p=b.points[i];if(p.stage>w.stage||Math.abs(p.x-w.anchor.x)>40||Math.abs(p.z-w.anchor.z)>38)continue;b.mesh.setMatrixAt(n,b.matrices[i]);const raw=b.placements[i].tint;if(raw){const values=raw.split(/[,\s]+/).map(Number);tint.setRGB(values[0]/255,values[1]/255,values[2]/255,THREE.SRGBColorSpace).multiplyScalar(values[3]??1);}else tint.set(0xffffff);b.mesh.setColorAt(n++,tint);}b.mesh.count=n;b.mesh.visible=n>0;b.mesh.boundingSphere=null;b.mesh.boundingBox=null;b.mesh.instanceMatrix.needsUpdate=true;if(b.mesh.instanceColor)b.mesh.instanceColor.needsUpdate=true;visibleInstances+=n;}};update();
 return {update,report:()=>({name:d.source.name,models,placements:[...grouped.values()].reduce((n,v)=>n+v.length,0),visibleInstances,area:d.stageAreas[w.stage-1],scale:d.source.worldUnitsPerSc2Unit,sourceSha256:d.source.sha256})};
}
