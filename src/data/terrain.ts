...
export class CharTerrain {
 ...
 height(p:Point){for(const r of RAMPS){const along=(r.axis==='x'?p.x-r.x:p.z-r.z)*r.sign,across=r.axis==='x'?p.z-r.z:p.x-r.x;if(Math.abs(across)<=r.width/2&&Math.abs(along)<=r.length/2)return CHAR_HEIGHT*clamp((along+r.length/2)/r.length);}return p.z<=-34||Math.abs(p.x)>=24?CHAR_HEIGHT:0;}
 region(p:Point){if(p.z<=-34)return 3;if(p.x>=24)return 1;if(p.x<=-24)return 2;return 0;}
 canOccupy(p:Point,r:number){const h=this.height(p),tolerance=r*.52+.035;for(const [x,z] of [[1,0],[-1,0],[0,1],[0,-1],[.707,.707],[-.707,.707],[.707,-.707],[-.707,-.707]])if(Math.abs(this.height({x:p.x+x*r,z:p.z+z*r})-h)>tolerance)return false;return true;}
 canStep(a:Point,b:Point,r:number){return this.canOccupy(b,r)&&Math.abs(this.height(a)-this.height(b))<=length(a,b)*.52+.02;}
 walkLine(a:Point,b:Point,r:number){const n=Math.max(1,Math.ceil(length(a,b)/.4));let old=a;for(let i=1;i<=n;i++){const p={x:a.x+(b.x-a.x)*i/n,z:a.z+(b.z-a.z)*i/n};if(!this.canStep(old,p,r))return false;old=p;}return true;}
 sameContactLayer(a:Point,b:Point){return Math.abs(this.height(a)-this.height(b))<.8;}
 lineOfFire(a:Point,b:Point,airA=false,airB=false,melee=false){if(melee)return this.walkLine(a,b,0);const start=airA?AIR_HEIGHT:this.height(a)+.75,end=airB?AIR_HEIGHT:this.height(b)+.75,n=Math.max(1,Math.ceil(length(a,b)/.4));for(let i=1;i<n;i++){const t=i/n,p={x:a.x+(b.x-a.x)*t,z:a.z+(b.z-a.z)*t};if(this.height(p)>start+(end-start)*t+.03)return false;}return true;}
