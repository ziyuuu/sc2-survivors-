...
export type LegacyEliteId=`${TerranType}.${1|2|3}`;
export type EliteId=LegacyEliteId|ExpansionEliteId;
export type EliteTemplate='quick'|'heavy'|'guard'|'mobile'|'support';
export const ELITE_TEMPLATES={
 quick:{output:1.7,as:1.5,hp:1.35,move:1.1,armor:0,dpsStep:.25,asStep:.1,hpStep:.12},
 heavy:{output:1.95,as:.95,hp:1.5,move:1,armor:.75,dpsStep:.3,asStep:.04,hpStep:.12},
 guard:{output:1.55,as:1,hp:2,move:1,armor:2,dpsStep:.15,asStep:.04,hpStep:.25},
 mobile:{output:1.65,as:1.25,hp:1.5,move:1.25,armor:.75,dpsStep:.2,asStep:.06,hpStep:.12},
 support:{output:1.55,as:1,hp:1.55,move:1.1,armor:.75,dpsStep:.25,asStep:0,hpStep:.15},
} as const;
export interface EliteDefinition {id:EliteId;family:import('./races').FamilyId;name:string;template:EliteTemplate;description:string;model:string;sourceModel:string;icon:string;effect?:ExpansionEliteDefinition['effect']}
...
export const LEGACY_ELITES:Record<LegacyEliteId,EliteDefinition>=Object.fromEntries([
