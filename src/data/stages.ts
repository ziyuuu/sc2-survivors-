...
export function stageConfig(stage:number,difficulty:Difficulty):StageConfig {const index=Math.max(0,Math.min(11,stage-1)),s=STAGES[index];if(difficulty==='normal')return {...s,difficulty,ambient:{...s.ambient},guards:{...s.guards}};
 const carry=emptyCounts();for(let i=0;i<index;i++)scaleCounts(STAGES[i].ambient,.5,carry);return {...s,difficulty,ambient:scaleCounts(s.ambient,.5,carry),guards:scaleCounts(s.guards,.5)};}
