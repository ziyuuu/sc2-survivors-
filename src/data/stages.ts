...
const ambient:Counts[]=[[6,0,0,0],[12,0,0,0],[28,0,0,0],[80,12,0,0],[120,24,0,0],[132,24,12,0],[160,32,16,0],[160,56,20,0],[192,48,40,0],[300,72,48,18],[360,96,72,30],[420,120,90,42]];
...
export const STAGES:StageConfig[]=names.map((name,i)=>({id:i+1,name,durationSeconds:i<3?120:i<9?240:360,ambient:ambient[i],guards:guards[i],entranceSpacing:i<3?.3:i<5?.8:.35,waves:[6,6,7,10,12,12,14,14,16,24,27,30][i],lingHp:[18,24,30][i]??35,speed:[1,1,1.1,1.1,1.12,1.12,1.15,1.15,1.18,1.18,1.2,1.2][i],width:[28,36,44,52,60,68,76,84,92,100,104,112][i],podHp:[600,900,1200,1500,1800,2100,2400,2400,2400,2400,2400,2400][i],reward:rewards[i],drones:i<2?4:i<3?2:i<9?3:4,eggs:i<2?3:2}));
...
export function stageSchedule(s:StageConfig,seed:number){const rng=seeded(seed+s.id*7919);
 const waves:Wave[]=Array.from({length:s.waves},(_,i)=>({at:s.id<=2?6+i*20+(rng()-.5)*2:12+i*(s.durationSeconds-24)/(s.waves-1)+(rng()-.5)*(s.durationSeconds-24)/(s.waves-1)*.3,types:[],bearing:rng()*Math.PI*2}));
 ...
 const events:EconomicSpawn[]=[...Array.from({length:s.eggs},(_,i)=>({at:s.id<=2?[20,54,82][i]+(rng()-.5)*4:s.durationSeconds*(.15+i*.5/Math.max(1,s.eggs-1)+(rng()-.5)*.06),kind:'egg' as const})),...Array.from({length:s.drones},(_,i)=>({at:s.id<=2?[14,38,74,98][i]+(rng()-.5)*4:s.durationSeconds*(.2+i*.6/Math.max(1,s.drones-1)+(rng()-.5)*.05),kind:'drone' as const}))].sort((a,b)=>a.at-b.at);
 return {waves,events};
}
