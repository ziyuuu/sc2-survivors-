...
export class Input {
 ...
 constructor(readonly world:World,readonly joystick:HTMLElement,readonly onPause:()=>void,battle?:BattlePointer){
  ...
  if(battle){const canvas=battle.canvas;this.cancelTap=()=>{const id=tap?.id;tap=null;if(id!==undefined&&canvas.hasPointerCapture(id))canvas.releasePointerCapture(id);};
