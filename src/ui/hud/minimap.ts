...
export class Minimap {
 ...
 update(){const w=this.world;this.element.hidden=!w.terrain?.definition||w.phase!=='battle'||w.paused;if(this.element.hidden)return;if(this.stage!==w.stage)this.background();const c=this.ctx,s=this.canvas.width;c.clearRect(0,0,s,s);c.drawImage(this.backdrop,0,0);const unit=s/180;
  ...
  this.canvas.title=`第 ${w.stage} 关 · 右键 / 轻点前往，点敌集火 · ${w.pods.filter(p=>p.status==='active').map(p=>SC2_UNITS[p.unitType].zh+' #'+p.id).join(' / ')}`;
 }
 private rescue(p:{x:number;y:number},glyph:string,unit:number){const c=this.ctx;c.fillStyle='#181e21';c.strokeStyle='#ffc277';c.lineWidth=unit;c.fillRect(p.x-4*unit,p.y-4*unit,8*unit,8*unit);c.strokeRect(p.x-4*unit,p.y-4*unit,8*unit,8*unit);c.font=`${7*unit}px sans-serif`;c.textAlign='center';c.textBaseline='middle';c.fillStyle='#ffe1a2';c.fillText(glyph,p.x,p.y+.5*unit);}
}
