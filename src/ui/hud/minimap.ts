...
export class Minimap {
 ...
 private background(){const d=this.world.terrain?.definition;if(!d)return;this.frame=minimapFrame(d,this.world.stage);this.stage=this.world.stage;this.mapTerrain=this.world.terrain;
 ...
 update(){const w=this.world;this.element.hidden=!w.terrain?.definition||w.phase!=='battle'||w.paused;if(this.element.hidden)return;if(this.stage!==w.stage||this.mapTerrain!==w.terrain)this.background();const c=this.ctx,s=this.canvas.width;c.clearRect(0,0,s,s);c.drawImage(this.backdrop,0,0);const unit=s/180;
  ...
  for(const e of w.entities.values())if(e.hp>0)dot(e,e.enemyTier==='lord'?'#ff514f':e.enemyTier==='boss'?'#ffb049':e.enemyTier==='elite'?'#ee6471':e.eliteId?'#ca82ff':e.owner==='terran'?'#8ceeaa':'#f96751',e.enemyTier==='lord'?5:e.enemyTier==='boss'?4:e.enemyTier==='elite'?3:e.owner==='terran'?2:1.5);
