...
export interface PadMapping {x:number;y:number;invertX:number;invertY:number;confirm:ButtonBinding;back:ButtonBinding;stim:ButtonBinding;siege:ButtonBinding;pause:ButtonBinding;previous:ButtonBinding;next:ButtonBinding}
export type PadSnapshot=Pick<Gamepad,'id'|'index'|'mapping'|'axes'|'buttons'|'connected'>;
export const STANDARD:PadMapping={x:0,y:1,invertX:1,invertY:1,confirm:{button:0},back:{button:1},stim:{button:2},siege:{button:3},pause:{button:9},previous:{button:6},next:{button:7}};
...
export class PadState {
 ...
 sample(p:PadSnapshot,m:PadMapping,now:number){const move=deadzone((p.axes[m.x]??0)*m.invertX,(p.axes[m.y]??0)*m.invertY);const edges=new Set<keyof PadMapping>();let pressed=false;
  for(const k of ['confirm','back','stim','siege','pause','previous','next'] as const){const v=value(p,m[k]),before=this.held.has(k);if(v>=.55){pressed=true;if(!before){edges.add(k);this.held.add(k);}}else if(v<=.35)this.held.delete(k);else pressed||=before;}
