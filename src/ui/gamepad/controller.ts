...
import {SKILLS,activateSkill,skillUnlocked,stickSkill,type SkillId} from '../controls/skills';
...
export class GamepadInput {
 ...
 direct(){if(!this.active)return;this.armed.clear();this.states.clear();this.active=false;this.hideSkills();this.world.input={x:0,z:0};this.world.cancelOrder();document.body.classList.remove('gamepad-active');}
 ...
 private select(p:PadSnapshot){this.active=true;this.index=p.index;this.world.cancelOrder();this.onActive();document.body.classList.add('gamepad-active');}
 poll(now=performance.now(),pads:readonly (PadSnapshot|null)[]=navigator.getGamepads?.()??[]){const connected=pads.filter((p):p is PadSnapshot=>!!p&&p.connected);if(this.active&&!connected.some(p=>p.index===this.index)){this.suspend();this.index=null;return;}
  ...
  for(const p of connected){const mapping=this.mappings[p.id]??(p.mapping==='standard'?STANDARD:null);if(!mapping){if(p.buttons.some(b=>b.value>.55)){this.openCalibration(p);return;}continue;}
   ...
   const w=this.world;if(w.phase!=='battle'||w.paused||w.requiresEliteChoice){w.input={x:0,z:0};this.hideSkills();this.menu(input.menu,input.edges.has('confirm')||input.edges.has('stim'),input.edges.has('back'));continue;}
   this.missingSkillAxes=p.mapping!=="standard"&&(mapping.skillX===undefined||mapping.skillY===undefined);w.input=input.move;
   this.selectedSkill=stickSkill(input.skill.x,input.skill.z,this.selectedSkill);
 ...
 private drawSkills(){
  const w=this.world,id=this.selectedSkill,selected=SKILLS.find(s=>s.id===id);
  ...
  for(const el of document.querySelectorAll<HTMLElement>('#skills button,#hero-skills button'))el.classList.toggle('pad-selected',el.dataset.action===id);
 }
 private menu(direction:string,confirm:boolean,back:boolean){const root=document.querySelector<HTMLElement>('#overlay');if(!root||root.hidden)return;const controls=[...root.querySelectorAll<HTMLButtonElement|HTMLSelectElement>('button:not(:disabled),select:not(:disabled)')].filter(e=>e.getClientRects().length);if(!controls.length)return;let i=controls.indexOf(document.activeElement as HTMLButtonElement),el=controls[Math.max(0,i)];
  if(back){const close=root.querySelector<HTMLButtonElement>('[data-action="settings-back"]');if(close){close.click();return;}(root.querySelector<HTMLButtonElement>('[data-action="skip"],[data-action="pause"]')??el).focus();return;}
