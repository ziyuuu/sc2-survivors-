...
test('ground contact stops overlapping movement while preserving the stationary defender',()=>{const w=fixture(),a=w.addUnit('marine','terran',0,0),b=w.addUnit('zergling','zerg',.56,0);a.prev={x:0,z:0};b.prev={x:.66,z:0};solve(w);assert.ok(Math.hypot(a.x-b.x,a.z-b.z)>=a.unitRadius+b.unitRadius-.001);assert.equal(a.x,0);assert.ok(b.x-b.prev.x<=.001);});
test('air can pass ground actors but same-layer aircraft have collision volume',()=>{const w=fixture(),g=w.addUnit('tank','terran',0,0),a=w.addUnit('medivac','terran',0,0);assert.equal(solve(w),0);assert.equal(g.x,0);assert.equal(a.x,0);const b=w.addUnit('medivac','terran',1.16,0);b.prev={x:1.26,z:0};solve(w);assert.ok(Math.hypot(a.x-b.x,a.z-b.z)>=a.unitRadius+b.unitRadius-.001);});
test('sieged tank is immovable and other units yield to its actual radius',()=>{const w=fixture(),t=w.addUnit('tank','terran',0,0),m=w.addUnit('marine','terran',.96,0);t.mode='siege';m.prev={x:1.06,z:0};solve(w);assert.equal(t.x,0);assert.ok(m.x>=t.unitRadius+m.unitRadius-.001);});
...
test('closed pod volume blocks ground troops without moving the pod',()=>{const w=fixture(),m=w.addUnit('marine','terran',1.51,0),p=w.spawnPod('marine',{x:0,z:0});w.landPod(p);m.prev={x:1.7,z:0};w.hash.rebuild([m,p]);resolveContacts([m],w.hash,[],w.mapHalf,1/60);assert.ok(m.x>=m.unitRadius+p.unitRadius-.001);assert.equal(p.x,0);});
...
test('radius-sized choke admits a Marine but blocks a Tank even when contact correction pushes it',()=>{
 const walls=[{x:0,z:2,w:5,h:3},{x:0,z:-2,w:5,h:3}],w=fixture(),m=w.addUnit('marine','terran',-4,0),t=w.addUnit('tank','terran',-4,0);m.facing=t.facing=Math.PI/2;
 ...
 assert.ok(m.x>2.5);assert.ok(t.x<=-2.5-t.unitRadius);assert.equal(blocked(t,t.unitRadius,walls),false);
