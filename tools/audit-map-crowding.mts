...
for(let ramp=Number(process.env.SC2_RAMP??1)-1;ramp<Number(process.env.SC2_RAMP??d.ramps.length);ramp++)for(const sign of [-1,1]){
 ...
 for(const type of ['marine','hellion','tank','medivac'] as const)for(let n=0;n<5;n++){const u=w.addUnit(type,'terran',start.x,start.z),p=safe(start.x+rx*(n-2)*1.9-dir.x*(['marine','hellion','tank','medivac'].indexOf(type)*2),start.z-ry*(n-2)*1.9-dir.z*(['marine','hellion','tank','medivac'].indexOf(type)*2),u.unitRadius);Object.assign(u,p,{prev:{...p},facing:Math.atan2(dir.x,dir.z)});}
...
const summary={cases:results.length,failed:results.filter(r=>!r.accepted||!r.settled||r.invalid).length};fs.writeFileSync(process.env.SC2_CROWD_OUT??'reports/local/map-crowding-current.json',JSON.stringify({at:new Date().toISOString(),method:'Five of each Terran type, 20 entities, real fixed-step World with collision and formation. Both directions at every original ramp, no combat or balance validation. The anchor stops 15 units beyond the ramp. Rear formation slots can legitimately remain before the ramp; only ground units still over 3 units from their own slot after 90 seconds count as unsettled. Starting positions are explicit diagnostic fixtures.',summary,results},null,2));if(summary.failed)process.exitCode=1;
