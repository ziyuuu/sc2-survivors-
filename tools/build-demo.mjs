...
for(const id of ['map.kairos','map.acropolis','map.terrain.diffuse','map.terrain.normal','map.terrain.mask0','map.terrain.mask1','map.acropolis.terrain.diffuse','map.acropolis.terrain.normal','map.acropolis.terrain.mask0','map.acropolis.terrain.mask1','model.loot.mineral','model.loot.gas','model.loot.large'])if(!records.some(r=>r.id===id&&r.status==='available'))throw Error('Original resource pickup required: '+id);
const resources=[];for(const r of records.filter(r=>r.status==='available')){const bytes=await fs.readFile(r.packedFile);const mime=r.kind==='map-data'?'application/json':r.kind==='map-model'?'model/gltf+json':r.kind==='model'?'model/gltf-binary':r.kind==='audio'?(r.packedFile.endsWith('.ogg')?'audio/ogg':'audio/wav'):r.packedFile.endsWith('.jpg')?'image/jpeg':'image/png';resources.push({id:r.id,mime,bytes});}
