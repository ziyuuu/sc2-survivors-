...
try{
 ...
 for(let i=0;i<600;i++){
  const s=await page.evaluate(()=>{const w=window.__SC2_DEBUG__.world;return {time:w.time,stage:w.stage,phase:w.phase,wave:w.wave,wallet:{...w.wallet},anchor:{...w.anchor},allies:w.allies().map(u=>({id:u.id,type:u.unitType,hp:u.hp,x:u.x,z:u.z,action:u.action})),enemies:[...w.entities.values()].filter(u=>u.owner==='zerg'&&u.hp>0).map(u=>({x:u.x,z:u.z,hp:u.hp,target:u.attackTarget,pod:u.guardianPod})),pods:w.pods.map(p=>({id:p.id,x:p.x,z:p.z,hp:p.hp,status:p.status,expiresAt:p.expiresAt,landedAt:p.landedAt,guards:p.guardianIds.size})),stats:{...w.stats},barracksReady:w.buildings.get('barracks')?.remaining<=0,rewards:w.rewards.map(r=>({id:r.id,name:r.name}))};});
