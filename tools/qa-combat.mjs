...
const veteran=process.env.SC2_QA_VETERAN==='1';
...
if(veteran)report.method=report.method.replace('Rank-1','Rank-4');
...
}catch(e){report.failure=String(e.stack??e);process.exitCode=1;console.error(report.failure);}finally{await fs.writeFile(`reports/local/qa/${rescueOnly?'COMBAT_RESCUE':'COMBAT_PLAYTHROUGH'}${veteran?'_VETERAN':''}.json`,JSON.stringify(report,null,2));await browser.close();}
