...
for layer in ['core','liberty','swarm','void']:
 ...
 for n in tree.getroot():
  ...
  old=nodes.get(n.attrib['id'],{'tokens':{},'parent':None,'model':None,'scale':None})
...
def resolve(name):
 ...
 tokens={'id':nodes[name]['parent'] if name in ['SiegeTank_CovertOps_Morph','SiegeTank_Commando_Morph'] else name};model=None;scale=None
 for n in chain:
  d=nodes[n];tokens.update(d['tokens']);model=d['model'] or model;scale=d['scale'] or scale
...
for key,name,path in [('raynor','MutatorAmonRaynor','MarineRaynorHEV_Ex3'),('tychus','MutatorAmonTychus','MarineTychus'),('nova','MutatorAmonNova','Nova_COOP')]:records.append({'id':'model.hero.'+key,'modelDataId':name,'assetPath':f'Assets/Units/Terran/{path}/{path}.m3','name':path.lower(),'sourceScale':None,'definition':'mods/starcoop/starcoop.sc2mod/base.sc2data/gamedata/modeldata.xml'})
