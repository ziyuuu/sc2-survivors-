...
const newPage=async(size={width:1440,height:900},mobile=false)=>{const ctx=await browser.newContext({viewport:size,isMobile:mobile,hasTouch:mobile,deviceScaleFactor:1});const page=await ctx.newPage();page.on('pageerror',e=>report.errors.push(e.message));page.on('console',m=>{if(m.type()==='error'&&!m.text().includes('404'))report.errors.push(m.text());});await page.goto('http://127.0.0.1:5173/');await page.waitForFunction(()=>window.__SC2_REPORT__?.().assetsReady,{timeout:60000});await page.getByRole('button',{name:/部署小队/}).click();return {ctx,page};};
try{
