...
import {HEROES,heroStats} from '../src/data/heroes';
...
function fireCapacity(w:World,stage:number,counts:number[],area=false){
 ...
 for(const u of w.allies()){
  if(u.unitType==='medivac')continue;const d=SC2_UNITS[u.unitType],rank=w.growth(u).damage,vehicle=w.upgrades.get('vehicle')??0;
...
async function budget(difficulty:Difficulty,plan:typeof plans[number]){
 ...
 for(let stage=1;stage<=12;stage++){
  w.stage=stage;w.stageStartedAt=w.time;w.prepareStage();const s=stageConfig(stage,difficulty),start=w.time,end=start+s.durationSeconds,budget=ZERG.map(t=>s.ambient[t]),rosterBefore=Object.fromEntries(TERRAN.map(t=>[t,w.allies().filter(u=>u.unitType===t).map(u=>u.rank)]));
  let hp=budget.reduce((n,count,i)=>n+count*enemyHp(ZERG[i],stage),0)+(stage===12?w.hive!.maxHp:0),singleCapacity=0,areaCapacity=0,landed=0,successes=0,failures=0;
  const schedule=stageSchedule(s,89241);schedule.waves.forEach(wave=>{const count=ZERG.map(()=>0);wave.types.forEach(t=>count[ZERG.indexOf(t)]++);events.push({at:start+wave.at+4,kind:'loot',m:count.reduce((n,k,i)=>n+k*DROPS.ambient[ZERG[i]][0],0)*plan.loot*f*ECONOMY.dropMultiplier,g:count.reduce((n,k,i)=>n+k*DROPS.ambient[ZERG[i]][1],0)*plan.loot*f*ECONOMY.dropMultiplier});});
  for(const e of schedule.specials){const factor=e.tier==='boss'?20:3,base=DROPS.ambient[e.type];events.push({at:start+e.at+15,kind:'loot',m:base[0]*factor*plan.loot*f*ECONOMY.dropMultiplier,g:base[1]*factor*plan.loot*f*ECONOMY.dropMultiplier});hp+=e.tier==='boss'?BOSSES[stage].hp:enemyHp(e.type,stage)*(e.type==='roach'?4:2);}
  ...
  for(let tick=0;tick<s.durationSeconds*60;tick++){
   ...
   for(const p of w.pods){if(p.status==='falling'&&p.landedAt<=w.time){p.status='active';landed++;guardsTotal+=p.guardTypes.length;hp+=p.guardTypes.reduce((n,t)=>n+enemyHp(t,p.stage),0);
    ...
    if(event.success){p.status='rescued';successes+=p.passengers.length;for(const c of p.passengers){w.reinforce(p.unitType,p);c.status='released';}for(const t of p.guardTypes)grant(DROPS.guard[t][0]*plan.loot*f*ECONOMY.dropMultiplier,DROPS.guard[t][1]*plan.loot*f*ECONOMY.dropMultiplier);}else {p.status='destroyed';failures+=p.passengers.length;for(const c of p.passengers)c.status='lost';}}
   if(tick%60===0){singleCapacity+=fireCapacity(w,stage,budget);areaCapacity+=fireCapacity(w,stage,budget,true);}
  }
  ...
  if(stage<12){w.phase='reward';w.rewardRound='building';w.rewardClaimed=false;
   const building=stage>=1&&!w.buildingsOf('factory').length?'factory':stage>=2&&!w.buildingsOf('starport').length?'starport':stage>=6&&w.buildingsOf('barracks').length<2?'barracks':null;
   const build=stage>=2&&!w.upgrades.has('marauder')?rewardPool(w).find(r=>r.id==='research.marauder'):building?rewardPool().find(r=>r.id==='build.'+building):null;w.rewards=build?[build]:[];if(build&&w.choose(build.offerId))chosen.push(build.id);w.skipReward();
