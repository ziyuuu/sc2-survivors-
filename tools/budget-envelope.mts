...
async function budget(difficulty:Difficulty,plan:typeof plans[number]){
 ...
 const resolving=new Map<number,{at:number;success:boolean}>();let collected={minerals:0,gas:0},guardsTotal=0,assumedLostRanks=0;
 ...
 for(let stage=1;stage<=12;stage++){
  ...
  const events:any[]=[],schedule=stageSchedule(s,89241);schedule.waves.forEach(wave=>{const count=[0,0,0,0];wave.types.forEach(t=>count[ZERG.indexOf(t)]++);events.push({at:start+wave.at+4,kind:'loot',m:count.reduce((n,k,i)=>n+k*DROPS.ambient[ZERG[i]][0],0)*plan.loot*f,g:count.reduce((n,k,i)=>n+k*DROPS.ambient[ZERG[i]][1],0)*plan.loot*f});});
  let eggs=0,drones=0;for(const e of schedule.events){if(e.kind==='egg'&&eggs++===0&&stage%plan.scvEvery===0)events.push({at:start+e.at+12,kind:'scv'});if(e.kind==='drone'&&drones++<Math.floor(s.drones*plan.droneFraction))events.push({at:start+e.at+12,kind:'loot',m:DROPS.drone[0]*f,g:DROPS.drone[1]*f});}events.sort((a,b)=>a.at-b.at);
  for(let tick=0;tick<s.durationSeconds*60;tick++){
   ...
   for(const p of w.pods){if(p.status==='falling'&&p.landedAt<=w.time){p.status='active';landed++;guardsTotal+=p.guardTypes.length;hp+=p.guardTypes.reduce((n,t)=>n+enemyHp(t,p.stage),0);
    ...
    if(event.success){p.status='rescued';successes++;w.reinforce(p.unitType,p);for(const t of p.guardTypes)grant(DROPS.guard[t][0]*plan.loot*f,DROPS.guard[t][1]*plan.loot*f);}else {p.status='destroyed';failures++;}}
  ...
  if(stage<12){w.phase='reward';w.rewardRound='building';w.rewardClaimed=false;
   ...
   if(r&&unlockedReward(w,r)){
    ...
    w.rewards=[r];if(w.choose(r.id)){chosen.push(r.id);if(!lab)nextCard++;}else w.skipReward();
   }else w.skipReward();
  }
