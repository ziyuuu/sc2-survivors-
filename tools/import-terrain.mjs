...
const textures=[['terrain.char','char_dirt.dds'],['terrain.char.normal','char_dirtnormal.dds'],['terrain.rock','char_rock.dds'],['terrain.rock.normal','char_rocknormal.dds'],['terrain.cracked','char_dirt_cracked.dds'],['terrain.cliff','char_cliff0.dds'],['terrain.cliff.normal','char_cliff0_nrm.dds'],['terrain.cliff.emissive','char_cliff0_emis.dds']];
...
for(const [id,name] of textures){const source='https://dist.sc2arcade.com/star-assets/textures/'+name,local='assets/private/dds/'+name;try{
 let bytes;try{bytes=await fs.readFile(local);}catch{bytes=await fetchBinary(source);}
