...
const out='reports/local/qa-v9-controls';await fs.mkdir(out,{recursive:true});
...
try{
 ...
 await fixture(p);await p.evaluate(()=>{const w=window.__SC2_DEBUG__.world;w.stage=8;w.prepareStage();w.eventPlan=[];w.anchor={x:0,z:0,facing:Math.PI/2};const u=w.allies()[0];u.x=0;u.z=0;u.prev={x:0,z:0};w.changed();});await p.waitForTimeout(700);const plateau=await clickWorld(p,{x:8,z:4});const high=await order(p);assert.equal(high.kind,'move');assert.ok(Math.hypot(high.point.x-8,high.point.z-4)<.15);report.desktop.plateau={pixel:plateau,point:high.point};
 await p.evaluate(()=>{const d=window.__SC2_DEBUG__;d.view.pick(innerWidth/2,innerHeight/2);d.world.stage=12;d.world.prepareStage();d.world.paused=true;d.world.anchor={x:88,z:112,facing:0};});await p.waitForTimeout(700);
 const bounds=await p.evaluate(()=>{const d=window.__SC2_DEBUG__;d.view.pick(innerWidth/2,innerHeight/2);const group=d.view.scene.getObjectByName('char-solid-cliff-faces');let checked=0,stale=0;group.traverse(m=>{if(!m.isInstancedMesh||!m.count)return;checked++;if(!m.boundingSphere)return;const matrix=m.matrix.clone();for(let i=0;i<m.count;i++){m.getMatrixAt(i,matrix);const s=m.geometry.boundingSphere.clone().applyMatrix4(matrix);if(s.center.distanceTo(m.boundingSphere.center)+s.radius>m.boundingSphere.radius+.01){stale++;break;}}});return {checked,stale};});report.desktop.expandedCliffBounds=bounds;console.log('Expanded cliff picking bounds',bounds);assert.ok(bounds.checked>0);assert.equal(bounds.stale,0);
