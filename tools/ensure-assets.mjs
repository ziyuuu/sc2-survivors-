...
const animationIds=['marine','hellion','tank','medivac','zergling','roach','baneling','ravager'].flatMap(t=>['model.'+t,'model.'+t+'.death']).concat(['model.tank.siege','model.tank.morph']);
if(animationIds.some(id=>!rows.some(r=>r.id===id&&r.status==='available'&&r.animations?.length&&r.materialPipelineVersion===2))||rows.filter(r=>r.kind==='effect-texture'&&r.status==='available').length<24){
