...
export function adaptM3Scene(group,sections,parser){
 ...
 for(const division of divisions){const batches=sections.getSectionByReference(division.batches)?.content??[],regions=sections.getSectionByReference(division.regions)?.content??[];
  regions.forEach((region,i)=>{const mesh=meshes[cursor++];if(!mesh)return;const batch=batches.find(b=>b.region_index===i);let ref=refs[batch?.material_reference_index];if(ref?.type===3){const composite=sections.getSectionByReference(sections.model.materials_composite)?.content?.[ref.material_index],parts=sections.getSectionByReference(composite?.sections)?.content??[];const base=parts.map(p=>refs[p.material_reference_index]).find(r=>r?.type===1);report.push({mesh:mesh.name,type:3,role:'composite',status:'base-converted',reason:'Original standard surface; dynamic creep overlay is inactive without game creep'});ref=base;}const spec=ref?.type===1?materials[ref.material_index]:null;
