...
**MVP表现重设计见[90精英/18英雄专项稿](MVP10_COMBAT_ASSETS.md)，M0 r6已批准、尚未实施。**用户要求模型特征及实际发出的弹道/命中突出身份，不接受统一紫色染色、巨大常驻姓名或放大施法圈。下文已有表现属于历史参考，样板和全量验收按MVP计划执行。目标规则仅玩家爆虫自爆后留场并停顿5秒，不能再给它播放原死亡动画；敌方仍用死亡自爆。[详细合同](MVP10_COMBAT_ASSETS.md#96-玩家爆虫自爆后停顿5秒)。
<!-- /MVP10_PLANNING_ENTRY -->

V24的边缘光和细环在混编队伍中仍不够醒目。V25在战斗镜头中为每位存活英雄增加从原素材清单读取的头像徽标，固定显示英雄名、生命和技能状态；雷诺、泰凯斯、诺娃分别有蓝／橙／紫色的宽地面信标。主动技能施放后1.2秒内短显施放者与技能名。英雄标签优先避让，精英继续使用较小的紫色次级标签；徽标与信标不截获触摸、鼠标或手柄指令。

这些都是视觉呈现，读取已有的模型、英雄记录和施放时间，没有改动伤害、视野、目标选择、资源或存档。Chrome headless已检验三名英雄同时参战时的头像加载、状态、信标与390×844边界；观感是否达到预期仍需玩家在实际战斗中确认，见 [QA](QA.md)。

---

# V24 诺娃装配与特殊单位视觉修复

诺娃步枪的纹理映射文件与本地导出图像核对一致，错位在模型装配：原GLB的 `Bone_Gun` 相对手部带有约0.89原始单位的局部偏移。运行时将枪骨放回A组步枪动作的手部参考位置，并只绘制当前步枪／身体／背包装备网格。未改M3、GLB或贴图字节，也没有以换图掩盖离手问题。

英雄、友军精英在原模型上分别叠加金色／紫色边缘光、双环与竖向标记，开火事件添加对应颜色的枪口闪光；名称样式提高对比度。命中和伤害仍完全由模拟结算。开发浏览器已核对真实模型、动作、可见文字标签和渲染错误；人工原客户端画面对照仍待进行。实际证据见 [V24 QA](QA.md)。

---

# V23 劫掠者修复记录

原始 `assets/private/m3/marauder.m3` 已与锁定CASC来源核对，SHA-256 `cb5c5fe1b6e5fcf6411187aa4fb7db68cb7a68aaf3579f4c7446f489d6240869`，715072字节，58骨骼／20片段；未替换或改写原始模型。

- 原Actor的WeaponNext初始0，出弹先加1；1播放Attack Right，回到0播放Attack Left。当前按每个实体实际出弹序号交替，普通与三款劫掠者精英共用规则。
- Ready对应原战斗姿态；行走事件清除两侧开火动作。渲染不以逻辑攻击恢复瞬间截断站定后的原动作。
- 原始节点 `Ref_Weapon Right/Left` 经GLTFLoader规范化为 `Ref_Weapon_Right/Left`，两种名称都解析；对应手的原骨骼轨迹控制出弹位置。
- 旧通用选择器只选第一个攻击片段，且两侧枪口都未绑定，回退到默认位置。真实浏览器验证现已存在两条轨迹，左右动作使用不同骨骼帧区间。
- 模拟伤害／射速不变。原M3/GLB来源与片段完整性、浏览器播放、人眼与原客户端逐帧对照是不同验收层级；后者仍待人工确认。

来源追踪：`tools/expansion-models.json`、`tools/expansion-dependencies.json`，本地锁定ActorData与当前 `render/loaders/animations.ts`。本地截图见V23 QA，未上传。

下面保留原有专题说明与历史版本记录；当前规则以 [DESIGN.md](DESIGN.md) 和本节为准。
...
本轮使用 10 个基础战斗模型、15 款原精英皮肤、3 位英雄，另有各自原死亡变体和坦克两态。精英来源系列及精确 Actor/Model 定义见 `tools/expansion-models.json`；劫掠者榴弹、刺蛇骨针和对应原效果依赖见 `tools/expansion-effects.json`。自定义名称、技能和强度不冒充原皮肤自带能力。

新资源从锁定 CASC 本地提取，运行 `npm run assets:expansion`；首次需要 README 所列 PowerShell/Python/CASC 依赖。下文旧八单位流程仍保留，但“不需要 Python”的描述仅适用于旧公开 M3 下载转换，不适用于新增 CASC 资源。

浏览器动作记录覆盖 28 款的站立/移动/死亡；23 款有普攻，4 款医疗艇使用 Stand Work 治疗，爆虫使用死亡自爆。诺娃遵循原 Actor 的 A 组动作。枪兵上身混合、原挂点出弹、原武器网格、坦克变形及医疗沿用现有渲染适配。原粒子发射时序、开舱门骨骼和英雄技能效果不是完整 SC2 特效引擎。

独立 M3Studio 结构读取比较 364 项模型、4045 个骨骼和 1272 段原动作时长，0 差异。原地面投影只有一个无权重静态骨骼，可烘焙进静态网格；它不是战斗单位骨骼。坦克 M3A 的 Flail 绑定不完整仍拒绝使用。数量、结构、播放、截图与人工还原批准分开记录，最终结果见 [QA.md](QA.md)。
...
按用户要求，八种战斗单位在既有 ModelData ×1.4 的导入归一化之外统一乘 0.8；原骨架、武器挂点、炮塔枢轴、死亡与变形共用同一归一化矩阵。实体碰撞半径也乘 0.8，原基础数据表与真实地图比例不变。生命条及接触影跟随新尺寸。没有把缩小后的效果称为原客户端 1:1 还原。
