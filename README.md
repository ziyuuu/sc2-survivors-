...
本轮缩短为 **12 关共 30 分钟**，增加开阔地横向编队、战斗侧向展开和较小的单位体积；稀有度卡牌、绿 Rank 3／蓝 Rank 5 即时队员、地图奖励，以及医疗艇机械维修升级已接入。继续使用真实 **Kairos Junction LE** 原地图与十二个递增区域，保留手动架炮、资源自动生产、救援与 PC 手柄。普通难度不以自动操作通关作为标准。详见 [验收](docs/QA.md) 与 [原地图管线](docs/MAP_PIPELINE.md)。
...
小地图显示当前开放区域、视野范围、小队、敌军、资源、虫巢及带兵种字母的救援仓（M/H/T/+，SCV 为 S）。右键小地图移动，左键／右键点敌集火；触屏轻点移动或集火。高台可向低地开火，近战不能隔崖攻击。此前拥堵坡口已按新编队复测，见 [当前验收](docs/QA.md)。
...
- 每个单位有独立 HP、护甲、射程、攻击周期、朝向与速度。枪兵短停开火后可移动；恶火有限转向和直线穿透；坦克由玩家统一架起／收起，行进模式也能开火；医疗艇连续治疗合法生物单位，购买纳米维修系统后可修复友方机械队员（不含自身和建筑）。
- 固定 60 Hz、开阔地最多三列展开、狭窄处收拢、独立行军拖尾、局部避让、实体碰撞、路径和有限追赶，不瞬移或整队同步平移。爆虫真实 AOE，破坏者胆汁可躲。
...
- 前两关首批在 2.5–3.5 秒到来，此后约每 10 秒一批，第一关 6 次单只跳虫，第二关普通为 6 次双只（简单共 9 只分摊 6 次）。两关均有 4 Drone 和 3 SCV 虫卵；击杀掉落仍需拾取，守军与 HP 不变。击杀掉落及每名 SCV 的采集增量提高 15%，基础自动采集与通关奖励不变。
- 1–3 关每关 60 秒，4–6 关 120 秒，7–9 关 180 秒，10–12 关 240 秒，共 30 分钟。地图逐关开放；第 12 关必须毁巢并活到结算。地面单位经坡道进入高台，空中独立高度层。
- 通关资源到账后依次选择建筑与随机支援，两轮均可买多项、买后刷新继续购买或直接继续。建筑可重复并各自生产。两轮共享 50／90／130…矿刷新费用；不按钱包过滤，不根据伤亡削敌，不免费回满 HP。

随机支援的单张稀有度为白 64%、绿 20%、蓝 10%、紫 5%、橙 1%；折扣另行生成。白色增援仍投仓；绿色队员直接获得 Rank 3，蓝色 Rank 5，满员时提升最低军衔至相应等级，不能挤掉已付款订单预留的容量。紫／橙当前为可生效的更强科技和推进技能，尚无特殊单位、英雄或装备。三张不重复，不按钱包过滤，已买卡不能再次扣款。

击杀战斗单位有 2%、Drone 有 8% 概率掉一份免费支援，靠近拾取；地图支援的白／绿／蓝／紫／橙为 84.8／10／4／1／0.2%。标识与小地图同色；已经无法培养的支援回收为该卡基础价格的矿气。原模型、动作及碰撞半径统一使用 80% 游戏尺寸，地图、射程与移速保持原比例。
...
node tools/qa-squad-rewards.mjs
