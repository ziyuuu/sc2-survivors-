...
本轮优化战场绘制与小队 AI：地图按完整模型范围裁剪屏幕外实例，减少重复索敌、编队计算和 GPU 上传；集火途中保留合法还击，医疗艇优先就近治疗并跟随行军。保留 **12 关共 30 分钟**、横向展开、稀有支援、真实 **Kairos Junction LE** 地图、手动架炮、自动生产救援与 PC 手柄。画质、固定模拟和关卡经济不变。详见 [实测验收](docs/QA.md) 与 [原地图管线](docs/MAP_PIPELINE.md)。
